...
@app.route('/view_bookings/<string:id>/')
def view_bookings(id):
	# # if request.method == 'POST':
	# cur = mysql.connection.cursor()
	# print("______________________________________")
	# result = cur.execute("SELECT * FROM bookings WHERE g_id = %d",[id])
	...
	# else:
	# 	msg = 'No bookings made by this guest'
...
def generate_bill(id):
	...
	#result = cur.execute("SELECT cost FROM charge c WHERE (c.type = (SELECT r_type FROM rooms r, bookings b2 WHERE r.r_id = b2.r_id AND b2.g_id = %s) AND code = 0) OR (c.type = (SELECT a_type FROM amenities a, bookings b2 WHERE a.a_id = b2.a_id AND b2.g_id = %s) AND code = 1)", (id, id))
	...
	#result = cur.execute("SELECT sum(cost) as total FROM charge c WHERE (c.type = (SELECT r_type FROM rooms r, bookings b2 WHERE r.r_id = b2.r_id AND b2.g_id = %s) AND code = 0) OR (c.type = (SELECT a_type FROM amenities a, bookings b2 WHERE a.a_id = b2.a_id AND b2.g_id = %s) AND code = 1)", (id, id))
